...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    $image       = '';
    $stocks = $_POST['stock'] ?? [];
    $colors       = sanitize($conn, $_POST['colors'] ?? '');
    $sizes        = sanitize($conn, $_POST['sizes'] ?? '');
    // Chuẩn hóa trước khi xử lý
    $sizes = array_map(function ($name) {
        ...
        if ($_POST['action'] === 'add') {
            ...
            if ($check->num_rows > 0) {
                ...
                foreach ($sizes as $s) {
                    ...
                    $check = $conn->query("SELECT id FROM sizes WHERE name='$s'");
                    if ($check->num_rows == 0) {
                        $newSizes[] = $s;
                    }

                    $size_ids[] = getOrCreateSize($conn, $s);
                }
                foreach ($colors as $c) {
                    $c = trim($c);

                    $check = $conn->query("SELECT id FROM colors WHERE name='$c'");
                    if ($check->num_rows == 0) {
                        $newColors[] = $c;
                    }

                    $color_ids[] = getOrCreateColor($conn, $c);
                }
                foreach ($size_ids as $size_id) {
                    foreach ($color_ids as $color_id) {

                        $conn->query("INSERT INTO product_varieties(product_id, size_id, color_id, stock_quantity, price)
                                    VALUES ($product_id, $size_id, $color_id, 0, 0)
        ");
                    }
                }
...
function renderSizeCheckboxes($conn, $existing_sizes_str)
{
    $result = $conn->query("SELECT size FROM sizes ORDER BY size");
    ...
    // extras (size ngoài DB)
    $extras = array_diff($selected, $sizes);

    $html .= '<input type="text" name="sizes_extra" class="form-control form-control-sm"
        placeholder="Thêm size khác, cách nhau bởi dấu phẩy (VD: 46,47)"
...
// Select or create id if not exists 
// SIZE
function getOrCreateSize($conn, $name)
{
    $name = strtolower(trim($name));

    $res = $conn->query("SELECT id FROM sizes WHERE size='$name'");
    if ($res->num_rows > 0) {
...
// COLOR
function getOrCreateColor($conn, $name)
{
    $name = strtolower(trim($name));

    $res = $conn->query("SELECT id FROM colors WHERE name='$name'");
    if ($res->num_rows > 0) {
        return $res->fetch_assoc()['id'];
    }
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-12">
                        ...
                        <?= renderSizeCheckboxes($conn, $edit_p['existing_sizes_str'] ?? '') ?>
                    </div>
                </div>
            ...
            <div class="card-body">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-12">
                            <label class="form-label">Size có sẵn</label>
                            <?= renderSizeCheckboxes($conn, $edit_p['available_sizes'] ?? '') ?>
                        </div>
                    </div>
