...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update'])) {
        ...
        $msg = '<div class="alert alert-success">Đã cập nhật giỏ hàng.</div>';
    }
    if (isset($_POST['remove'])) {
        $idx = (int)$_POST['remove'];
        array_splice($_SESSION['cart'], $idx, 1);
        $msg = '<div class="alert alert-info">Đã xóa sản phẩm khỏi giỏ hàng.</div>';
    }
}
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <button type="submit" name="remove" value="<?= $i ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Xóa sản phẩm này?')">
